#Info on the templete for adding new tts engines
#will be at libs/classes/tts_engines/.template.py
#tags: NEW_TTS must be replace by the tts engine name key  from TTS_ENGINES in models.py
#all NEW_TTS.XXX must be replaced by the tts engine function equivalent of coqui. so NEW_TTS.LOAD_CHECKPOINT etc..
#the proper NEW_TTS import must be inside the _load_api and _load_check_point functions
#that's it
